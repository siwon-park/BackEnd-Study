...
스프링 빈은 컴파일 시점에 생성 및 등록되지 않는다. 위의 빈 라이프 사이클을 보면 알겠지만 런타임 시점에 스프링 컨테이너가 생성되고 나서 생성 및 등록된다.

따라서 만약에 @Autowired로 필드 주입을 할 때, 주입할 타입이 스프링 빈으로서 존재하지 않더라도 IDE 상에서는 오류가 발생하겠지만, 컴파일이나 빌드에는 아무 문제가 없다.

- TestService 인터페이스를 구현한 클래스에 어떠한 @Componet 어노테이션도 붙이지 않음

![image-20250501165614462](assets/image-20250501165614462.png)

- 컴파일 및 빌드 시에는 문제가 없음

![image-20250501165812289](assets/image-20250501165812289.png)

- 그러나 런타임 시점에 해당 타입의 빈을 등록하여 주입할 수 없어 에러가 발생함

![image-20250501165704956](assets/image-20250501165704956.png)
