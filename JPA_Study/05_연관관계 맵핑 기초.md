...
테이블을 있는 그대로 객체화하여 모델링한다.

#### (1) 예시

teamId라는 Long 타입 외래키 필드를 직접 둬서 맵핑하는 경우
...
즉, 객체를 테이블에 맞추어 데이터 중심으로 모델링하면 위 예시처럼 객체 간 협력 관계를 만들 수 없다.

- 객체 간 협력 관계란 다른 객체의 부모 / 자식 등 직접적인 참조를 하는 것을 포함하는 관계를 말한다.

객체 지향의 관점에서 보면 어떤 객체는 다른 객체의 부모나 자식이 될 수 있어 바로 참조를 할 수 있는데, 이렇게 객체를 단순히 테이블에 맞추어서 데이터 중심적으로 모델링을 하게 되면 객체 지향적이라고 볼 수 없다.

### 3) 연관 관계 없이 다른 객체의 필드로서만 사용되는 경우

#### (1) 예시

연관 관계 어노테이션 없이 다른 객체의 필드로 둔 경우
...
이 경우 team 필드는 단순히 타입이 Team인 Java 객체 필드이다. 따라서 Member와 Team의 관계는 Java 객체 간의 관계일 뿐 데이터 베이스 상에서는 아무런 연관 관계도 없게 된다.

JPA와 같은 ORM에서 제공하는 강력한 기능을 사용하지 못하게 된다.
