...
## 8. Spring Data JPA 버전별 null 처리 방식

Spring Data JPA의 버전에 따라 findBy 메서드의 파라미터에 null을 넣었을 때의 차이점이 존재한다.

### 1) Spring Data JPA 2.0 이전

null을 literal로 처리했다. 즉, findBy 메서드 호출 시 `WHERE column1 = null`로 쿼리가 호출되어 결과가 항상 없음이었다.

단, 결과가 없다는 의미는 반환 값이 null이라는 의미를 가지지는 않는다. 반환 타입에 따라 리턴하는 결과에 차이가 있다.

- 컬렉션(List 등)의 경우, 결과값이 없을 때는 빈 리스트([ ])를 반환한다. (null이 아님)
  - **※ Spring Data JPA는 컬렉션 반환 시에는 절대 null을 반환하지 않고, 항상 빈 컬렉션을 반환**한다.
- Optional의 경우, 결과값이 없을 때는 Optional.empty()를 반환한다.
- 그 외의 일반 객체의 경우, 결과값이 없을 때는 null이다.

<br>

### 2) Spring Data JPA 2.0 이후

null을 조건으로 처리한다. 즉, findBy 메서드 호출 시 자동으로 null을 `WHERE column1 IS NULL`로 변환하여 쿼리가 수행된다. null인 데이터를 찾아서 반환하기 때문에 결과 값이 있을 수도 있고, 없을 수도 있다.

따라서 만약에 정말로 데이터가 null인 것들을 찾기 위해 의도한 것이라면 상관 없지만 그렇지 않을 경우에는 null에 대한 분기 처리를 해줘야 한다.

```java
// 검색어가 없으면 전체 조회하려는 의도
...
// 실수 발생(의도하지 않는 결과를 반환하는 코드)
public List<User> searchByKeyword(String keyword) {
    return userRepository.findByName(keyword);  // 데이터가 null인 값들을 찾아 반환
}
```

또는 다음과 같이 null 조건에 대해서 명시적으로 처리하는 방식이 권장되기도 한다.

```java
// 명시적으로 IS NULL 조건
List<User> findByNameIsNull(); // 파라미터 없음. IS NULL인 조건을 만족하는 데이터를 찾기 위한 목적
