...
※ 클래스도 함수와 마찬가지로 public이 default이다.
...
※ 아래 예시 사진들은 변수에 대한 값 초기화는 이뤄지지 않은 사진임
...
| constructor 키워드 사용                                      | constructor 키워드 생략<br />클래스 중괄호 선언              | 클래스 선언부                                                |
...
부 생성자는 있을 수도 있고 없을 수도 있다. => `constructor`키워드를 통해 생성(**생략 불가!!**)

body를 가질 수 있으며,`this`키워드를 통해 부 생성자는 다른 부 생성자를 호출할 수도 있고, 최종적으로는 주 생성자를 호출해야한다.

**※ init과 부생성자가 있을 때, init이 먼저 호출된다!**
...
※ IntelliJ에서`Person()`을 Ctrl + 클릭하여 선언부로 이동하면 `두 번째 부 생성자`로 이동하고, 두 번째 부 생성자에서 `this`를 클릭하면 `첫 번째 부 생성자`로 이동하고, 첫 번째 부 생성자에서 `this`를 클릭하면 `주 생성자`로 이동한다.

### 결론

> 부 생성자를 생성해서 사용하는 것보다 다음과 같이 default parameter를 사용하는 것이 간결하다.

아래 예시는 클래스 선언부에서 name과 age에 대한  default value를 초기화하였음
...
코틀린 생성자 개념 이해를 위한 추가 예시

연두색 밑 줄은 주 생성자(primary constructor)이고, 아래 constructor 키워드로 선언된 부분은 부 생성자(secondary constructor)이다.

하늘색 밑줄로 선언된 필드인 hobby는 주 생성자에 들어가는 요소는 아니지만, 기본적으로 "과학"이라는 값을 갖고 있다.

- 만약 주 생성자로 User 인스턴스를 초기화하면 해당 인스턴스의 hobby 값은 `"과학"`이다.
- 부 생성자로 User 인스턴스를 초기화 할 때, hobby에 대한 값을 지정해줄 경우, 해당 인스턴스의 hobby 값은 부 생성자를 통해 hobby에 전달한 값을 갖게 된다.
  - 이 때, 부 생성자의 `this(name, age)`는 결국 주 생성자를 호출한 것이다.
  - 즉, 부 생성자를 통해 인스턴스를 생성하면 hobby에 전달한 값으로 hobby가 초기화되고, 주 생성자를 호출해서 name과 age에 대한 값을 받는 것이다.

![image](https://user-images.githubusercontent.com/93081720/206668508-30760d32-d586-4a17-8d93-245b1ad03fbf.png)

### init

초기화 구문; 생성자가 호출되는 시점에 호출 => 생성자에서 검증 로직을 만들 수 있게 해줌

![image](https://user-images.githubusercontent.com/93081720/198883055-3b26e6d0-8758-4ab7-a7c5-23e4e177ebc1.png)

age가 0보다 작은 경우와 age가 20미만인 경우에 대한 검증 로직으로 init 구문이 사용되었음
